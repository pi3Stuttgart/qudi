...
def LinearRetarder(phi2, delta,shift=0):
    """ General linear retarder, phi is the angle of the retarder
        ...
        Retardation is specified in units of degrees, waves, or nanometers. 
        One full wave of retardation is equivalent to 360, 
        or the number of nanometers at the wavelength of interest. 
        Tolerance on retardation is typically stated in degrees, 
        natural or decimal fractions of a full wave, or nanometers. 
    """
    phi2 = phi2-shift
    #print 'shift: ',shift
    return map(lambda phi: np.array(
...
def FitStokes(x,y,q=True, start_angle=0,units='degree', title='', cross=True, plotcolor='bo', label=''):
    ...
    pars['circular'].set(value = 0.2,min=0, max=1.5)#,expr='1-linear'
    if q:
        pars['linear'].set(value = 0.0,min=0, max=1.5)#,expr='1-circular'
...
def FitStokesDictionary(laserdict,PLdict,q=True ,plottitle=''):
    ...
    for key, value in PLdict.iteritems() :
        if not key == 'angle':
            x,y,sh,result, fitlabel= FitStokes(laserdict['laser_angle'],laserdict['laser'],q=q, start_angle=0,units='degree', title=plottitle, cross=True, plotcolor='go', label='laser')
            laser_shift = sh
            print 'shift', sh
            corrected_angle = (PLdict['angle']-laser_shift*45/np.pi)*np.pi/90.
            # fit laser again with right shift
            x,y,sh,result, fitlabel= FitStokes(corrected_angle*90/np.pi,laserdict['laser'],q=q, start_angle=0,units='degree', title=plottitle, cross=True, plotcolor='go', label='laser')
            #plot it
            PlotStokes(corrected_angle,y,result,sh,fitlabel, plotcolor='y',title=plottitle, alpha=0.5, line_label='785nm laser')
            color=returnPlotcolor(key)#to have different colors
            #print 'color', color
            x,y,sh,result, fitlabel= FitStokes(corrected_angle*90/np.pi,value,q=q, start_angle=0,units='degree', title=plottitle+key, cross=True, plotcolor='go', label='laser')

            aplot=PlotStokes(corrected_angle,y,result,sh,fitlabel, plotcolor=color,title=key+plottitle,line_label=key)
...
# ------- Jones Formalism --------------
# ---------------------------------------
def rot(phi2, shift=0):
    """Rotation Matrix for Jones nomenclature."""
    return map(lambda phi: [[np.cos(phi-shift), -np.sin(phi-shift)], [np.sin(phi-shift), np.cos(phi-shift)]],phi2)
def qwpm(phi):
    """Qarter Waveplate Jones: with fast axis at angle phi w.r.t the horizontal axis"""
    j=np.complex(0,-1)
    return np.exp(j*np.pi/4)*np.array([[np.square(np.cos(phi))+j*np.square(np.sin(phi)),(1-j)*np.sin(phi)*np.cos(phi)], [(1-j)*np.sin(phi)*np.cos(phi), np.square(np.sin(phi))+j*np.square(np.cos(phi))]])
def JonesPolarization(x,shift=0.3, rcircular=0.5, vlinear = 0.5, lcircular=0.5, hlinear = 0.5, qw=1):
    """takes 8ms for 629 values on intel I5
       x        = values in radians, 0-2 pi
       shift    = shift in radians
       circular = amount of circular light
       linear   = amount of linear light
       qw       = if quarter waveplate is used in detection, else half waveplate
    """
    # create models according to Rolands Mathematica file
    j=np.complex(0,-1)#first number real part, second imaginary part
    lhp   = np.array([1,0])#horizontally polarized linear light
    lvp   = np.array([0,1])#vertically polarized linear light
    lplus = 1/np.sqrt(2)*np.array([1,1])
    lminus= 1/np.sqrt(2)*np.array([1,-1])
    rcp   = 1/np.sqrt(2)*np.array([1,j])# right handed circular light
    lcp   = 1/np.sqrt(2)*np.array([1,-j])# right handed circular light
    polh  = np.array([[1, 0] , [0, 0]])#horizontal polarization
    hwp   = np.array([[-j, 0], [0, j]])#half waveplate
    idy   = np.identity(2) #identity matrix
    qwp  = np.exp(-j*np.pi/4)*np.array([[1, 0], [0, j]])#quarter waveplate
    rm   = rot(-x/2,shift)#rotation negative
    rp   = rot(x/2,shift)#rotation positive
    #qwp = qwpm(shift)
    # mixture of circular and linear light
    l = vlinear # vertical linear component
    c = rcircular#right circular component
    cl= lcircular#left circular component
    lh= hlinear# horizontal linear component
    mixture = c*rcp+l*lvp+cl*lcp+lh*lhp
    #--- need to split function that matrix multipl works:
    if qw==1:# if a qwp is used in detection
        first = np.einsum('...ij,...jk->...ik',rm ,qwp)
    else: # if a hwp is used in detection
       #print 'hwp detected'
        first = np.einsum('...ij,...jk->...ik',rm ,hwp)

    second = np.einsum('...ij,...jk->...ik',first,rp)#einsum for matrix multiplication
    function = np.einsum('...j,j',second,mixture) # einsum for multiplication of matrix and
    ix = np.absolute(np.einsum('j,...j',lvp,function))**2
   
    return ix#return only one side of the PBS
def FitJonesPolarization(x,y,q=True, start_angle=0,units='degree', title='', cross=True, plotcolor='bo', label=''):
    #start_angle=0,units='degree', title=plottitle, cross=True, plotcolor='go', label='laser'
    degree_sign= u'\N{DEGREE SIGN}'# to have the degree sign
    from lmfit import  Model
    norm = np.max(y)
    y = y/norm #normalize it
    ...
    if units == 'degree':
        x = x*np.pi/90.
        start_angle = start_angle*np.pi/90.
    polmodel=Model(JonesPolarization)
    pars = polmodel.make_params()
    pars['shift'].set(value = 0, min=-4*np.pi,max=3*np.pi)
    pars['vlinear'].set(value = 0.5,min=0, max=1.5)
    pars['rcircular'].set(value = 0.5,min=0, max=1.5)
    pars['lcircular'].set(value = 0.5,min=0, max=1.5,expr='1-rcircular-vlinear')#
    pars['hlinear'].set(value = 0.5,min=0, max=1.5,expr='1-vlinear')#-rcircular-lcircular
     
    pars['qw'].set(value=qw, vary=False)
    result = polmodel.fit(y,pars,x=x,qw=qw)
    #some parameters
    sh=(result.best_values['shift']-2*start_angle)-0.25*np.pi %np.pi#modulo pi
    #sh=result.best_values['shift'] %np.pi#modulo pi
    #print 'shift ', sh
    rcp= np.round(result.best_values['rcircular'],2)
    lvp= np.round(result.best_values['vlinear'],2)   
    lhp= np.round(result.best_values['hlinear'],2)
    lcp= np.round(result.best_values['lcircular'],2)   
    fitlabel = 'fit ('+'vlin= '+str(lvp)+', hlin= '+str(lhp)+','+ r' $\circlearrowright$ '+'='+str(rcp)+','+ r' $\circlearrowleft$ '+'='+str(lcp)+' )'
    return x,y,sh,result, fitlabel,rcp,lvp,lhp,lcp

def FitJonesDictionary(laserdict,PLdict,q=True ,plottitle=''):

    i=0
    for key, value in PLdict.iteritems() :
        if not key == 'angle':
            x,y,sh,result, fitlabel,rcp,lvp,lhp,lcp= FitJonesPolarization(laserdict['laser_angle'],laserdict['laser'],q=q, start_angle=0,units='degree', title=plottitle, cross=False, plotcolor='go', label='laser')
            PlotJones(x,y,result,sh,fitlabel, plotcolor='y',title=plottitle, alpha=0.5, line_label='785nm laser')
            color=returnPlotcolor(key)#to have different colors
            #print 'color', color
            x,y,sh,result, fitlabel,rcp,lvp,lhp,lcp = FitJonesPolarization(PLdict['angle'],value,q=q, start_angle=0,units='degree', title=plottitle+key, cross=True, plotcolor='go', label='laser')
            aplot=PlotJones(x,y,result,sh,fitlabel, plotcolor=color,title=key+plottitle,line_label=key)

            #axarr
            plt.savefig(key+'jones_fit.png')
            pp = PdfPages(key+'jones_fit.pdf')
            pp.savefig()
            pp.close()
            plt.clf()
            plt.close()
            i+=1
    return rcp,lvp,lhp,lcp
def PlotJones(x,y,result,sh,fitlabel, plotcolor, title,cross=True, alpha=1, line_label=''):
    degree_sign= u'\N{DEGREE SIGN}'# to have the degree sign
    plt.subplots_adjust(bottom=0.35)
    ax=plt.subplot(111,projection='polar')
    ax.plot(x,y, plotcolor+'o',linewidth=3,label=line_label+' raw data')
...
#cal='laser_calibration 5.5K HWP_360 13_degree B'
#calibration_path= base+date+sample+meas+'/First_Measurement/pys'
...
while i >= 0:
    ...
    #v1 = np.sum(intensity[477:488])
    v1 = np.sum(intensity[200:900])
    ...
    v1prime = np.max(intensity[463:475])
...
#ax2.plot(angle_array*np.pi/90.,v1norm, color='r', linewidth=3, label='v1')
#ax2.plot(angle_array*np.pi/90.,v1primenorm, color='b', linewidth=3,label='v1_prime')
...
#ax3.plot(angle_array,v1norm, color='r', linewidth=3,label='v1')
#ax3.plot(angle_array,v1primenorm, color='b', linewidth=3,label='v1_prime')
...
ax5.plot(angle_array*np.pi/90., v2norm, color='b', linewidth=3,label='v2')
...
#FitStokesDictionary(PLdict,q=q ,plottitle=plottitle)
#rcp,lvp,lhp,lcp = FitJonesDictionary(PLdict,q=q ,plottitle=plottitle)
# combine the plots into one
import shutil
import pdfkit #download it here:  http://wkhtmltopdf.org/downloads.html

def create_template_html_jones(stringle,datapath,startfolder):
    b = startfolder+'/tools/'
    s="<h1>"+stringle+"</h1>"
    s1="<h6>"+datapath+"</h6>"
    with open(b+"template2.html") as infile:
        with open(b+"template_jones.html","w") as outfile:
            for i,line in enumerate(infile):
                ...
                if i==30:
                    ...
                    outfile.write(line)
def create_template_html_stokes(stringle,datapath,startfolder):
    b = startfolder+'/tools/'
    s="<h1>"+stringle+"</h1>"
    s1="<h6>"+datapath+"</h6>"
    with open(b+"template.html") as infile:
        with open(b+"template_stokes.html","w") as outfile:
            for i,line in enumerate(infile):
                if i==8:#8th line
                    outfile.write(s+s1)
                if i==30:
...
def convertAllToPdf(startfolder):
    ...
    shutil.copy('v1primejones_fit.png', b)
    shutil.copy('v2primejones_fit.png', b)
    shutil.copy('v1jones_fit.png', b)
    shutil.copy('v2jones_fit.png', b)
    shutil.copy('Spec_Polar_P5.5K_Emission_Polarisation.png',b)
    pdfkit.from_file(startfolder+'/tools/template_stokes.html', g+'_stokes_all.pdf',configuration=config)
    pdfkit.from_file(startfolder+'/tools/template_jones.html', g+'_jones_all.pdf',configuration=config)
    ...
    os.remove(b+'v1primejones_fit.png')
    os.remove(b+'v2primejones_fit.png')
    os.remove(b+'v1jones_fit.png')
    os.remove(b+'v2jones_fit.png')


#create_template_html_jones(date,data_path,startfolder)
create_template_html_stokes(date,data_path,startfolder)
#convertAllToPdf(startfolder)
#shutil.copy(g+'_stokes_all.pdf',base+date)
#shutil.copy(g+'_jones_all.pdf',base+date)

#---- open Windows explorer automatically
import subprocess
