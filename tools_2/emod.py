"""
The execution model. needs to be replaced by qudi state machine.
"""

import threading
import logging
import os, sys, inspect
from tools_2.utility import Singleton, StoppableThread, timestamp

#from traits.api import HasTraits, Instance, Enum, Range, Button, Str
...
class FreeJobHandler( Handler ):

    def object__title_changed(self,info):
...
class FreeJob( Job ):
    ...
    title = Str('Mission: '+os.path.basename(os.path.splitext(inspect.getouterframes(inspect.currentframe())[2][1])[-1]).replace('_',' '))#[2][1])[0]
