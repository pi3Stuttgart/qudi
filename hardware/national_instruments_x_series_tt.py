...
class NationalInstrumentsXSeries(Base, SlowCounterInterface, ConfocalScannerInterface, ODMRCounterInterface):
    ...
    counter = Connector(interface = "TimeTaggerCounter")
    def on_activate(self):
        """ Starts up the NI Card at activation.
        """
        self._timetagger = self.timetagger()
        self.counter() #initialize the timetagger counter
    ...
    def _set_up_line(self, length=100):
        ...
        params = {
            'count_channel': self._timetagger.get_combined_channels(), 
            'start_channel': self._pixel_start_ch, 
            'stop_channel': self._pixel_stop_ch, 
            'n_values': self._line_length
        }
        self._timetagger.set_count_between_markers(params)
    ...
    def scan_line(self, line_path=None, pixel_clock=False):
        ...
        try:
            ...
            while (time.time()-timeout)<(1/self._count_frequency * 20):
                pass
            counts = np.nan_to_num(self._timetagger.get_count_between_markers())
