...
class streamUSBnidaq(Base, StreamUSBNidaqInterface): #Hardware file
    ...
    chan_in_current = ConfigOption('chan_in_current', False, missing='warn') #test if readout from config file works
    ...
    _buffer_in_size = ConfigOption('buffer_size', 10, missing='warn')
    _sampling_freq_in = ConfigOption('sampling_frequency', 100, missing='warn')
    ...
    def start_acquisition(self):
        ...
        print("chan_in", self.chan_in)
        print("chan_in_current", self.chan_in_current)
        if self.chan_in_current != 'None':
            print("chan_in", self.chan_in)
            print("chan_in_current", self.chan_in_current)
            self.task_current_in = nidaqmx.Task(new_task_name="CurrentTask")
            self.cfg_current_read_task(self.task_current_in)
            self.stream_current_in = AnalogMultiChannelReader(self.task_current_in.in_stream)
            self.task_current_in.register_every_n_samples_acquired_into_buffer_event(self.bufsize_callback, self.reading_current_task_callback) 
            ...
            self.task_current_in.start()


        if self.chan_in != 'None':
            print("chan_in", self.chan_in)
            print("chan_in_current", self.chan_in_current)
            self.task_in = nidaqmx.Task(new_task_name="VoltageTask")
            self.cfg_voltage_read_task(self.task_in)
    ...
    def cfg_voltage_read_task(self, acquisition):
    ...
    def cfg_current_read_task(self, acquisition):
        acquisition.ai_channels.add_ai_current_chan(self.chan_in_current)
        acquisition.timing.cfg_samp_clk_timing(rate=self.sampling_freq_in, sample_mode=constants.AcquisitionType.CONTINUOUS,
                                            samps_per_chan=self.buffer_in_size_cfg)
        # print("Connected read task.")
    ...
    def reading_current_task_callback(self, task_idx, event_type, num_samples, callback_data):
        #print("buffer in", self.buffer_in)
        #print("num samples", num_samples)
        self.stream_current_in.read_many_sample(self.buffer_in, num_samples, timeout=constants.WAIT_INFINITELY)
