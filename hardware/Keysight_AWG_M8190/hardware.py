...
class SequencerTable(collections.OrderedDict):
    ...
    def data(self):
        ...
        for i in it[:-1]:
            ...
            if 2147483648 in out[-1][:, 0]: #2147483648=2^31
                break
        else:
            out.append(self.read_data(it[-1], block_size-1).reshape(-1, 6)) #CHANGED added reshape
    ...
    def reset(self, sequence_id=None, number_of_entries=None):
        if sequence_id is None:
            ...
            #data = self.data
            n = len(self.data) - sequence_id if number_of_entries is None else number_of_entries
            nbytes = str(24 * n)
            bytes_length = str(len(nbytes))
            cmd = '{},{}'.format(sequence_id, '#' + bytes_length + nbytes)
            for _ in range(n):
        ...
        try:
            ...
            for i, step in enumerate(sequence.data_list):
                if hasattr(step, 'reused_sequence_step'):
                    ...
                    cmd = bytes('TRAC%i %i,0,' %(self.channel_number, wm_segment.sid),encoding='utf8') #CHANGED: added ,encoding='utf8'
                    length = step.length_smpl*2
                    cmd += bytes('#' + str(len(str(length)))+str(length), encoding='utf8')#CHANGED: added ,encoding='utf8'
            ...
            raise Exception('An error occured while writing sequence {}, sequencestep ({},{})'.format(
                sequence.name, step.name, i))
        #print(len(sequencer_table_data))
        self.sequencer_table.data = sequence.sequence_table_data_block(
            segment_ids,
            sequence_id_offset=0)#len(sequencer_table_data)-1)#=0

    def delete_sequence(self, sequence):
        segments_catalogue = np.array(self.segments.catalogue, dtype=int)
        #sequences_catalogue = self.sequencer_table.catalogue() #FIXME #TODO return when the pci is installed.
        for i, step in enumerate(sequence.data_list):
        ...
        try:
            pass
            # We bypassed the long reading of the catalogue to save time, please return when the pci is installed.
        ...
        self.sequencer_table.reset(sequence_id=0, number_of_entries=1)
        for i, step in enumerate(sequence.data_list):
            if not hasattr(step, 'reused_sequence_step'):
    ...
    def query(self, cmd):
        ...
        try:
            ...
            if errors != '':
                print('query to awg, cmd: ', cmd)
                print('query res ', res)
                print('previous command: ',self.previous_comand)
                print('previous res: ',self.previous_res)
                raise Exception('Errors occurred while querying AWG:\n' + errors)
            else:
                raise inst
        finally:
            self.previous_comand = cmd
...
class PulseStreamer():
    ...
    def stream(self, seq, n_runs=0, initial=(0,[],0,0), final=(0,[],0,0), underflow=(0,[],0,0), start='IMMEDIATE'):
        ...
        self.proxy.stream(s, n_runs, initial, final, underflow, start)

    def startNow(self):
        self.proxy.startNow()

    def constant(self, pulse=(0,[],0,0)):
    ...
    def enc(self, seq):
        ...
        s = b''
        for pulse in seq:
            s+=struct.pack('>IBhh', *self.convert_pulse(pulse))
        return base64.b64encode(s)

    def convert_pulse(self, pulse):
        t, chans, a0, a1 = pulse
        return (t, self.chans_to_mask(chans), int(0x7fff*a0), int(0x7fff*a1))

    def chans_to_mask(self, chans):
        mask = 0
        for chan in chans:
            mask |= 1<<self.channel_map[chan]
        return mask
...
class PulseStreamerPGProxy(PulseStreamer):

    def __init__(self, **kwargs):
        PulseStreamer.__init__(self, **kwargs)
        self._loops     = None
    ...
    def missing_smpl(self,ls):
        return float(max(5 * 64 - ls, (-ls) % 64))
    ...
    def unwrap_sequence(self,sequence):
        ...
        for idx,item in enumerate(sequence[::-1]):
            if len(item) == 2:
                ...
                segment.insert(0, ([],
                                   np.round(
                                       el.length_smpl2length_mus(
                                           self.missing_smpl(
                                               el.length_mus2length_smpl(length * 1e-3)))*1e3,3),
                                    )
                               )
        ...
        if len(segment) > 0:
            for step in segment:
                seq.append(step)
        return seq[::-1] #should be flipped.


    def setSequence(self, sequence, loops=-1, triggered=False, padding=[], run=True):
        if run == True:
            start = 'IMMEDIATE'
        elif run == False:
            start = 'SOFTWARE'
        ...
        #self.stream(self.convert_sequence(sequence), loops, initial=(self.t_prepolarize, ['laser'], 0, 0), final=(100, [], 0, 0), start=start)

        self.stream(self.convert_sequence(sequence), loops, final=(100, [], 0, 0), start=start)


    def run(self, loops=-1, triggered=False):
        if loops != self._loops or triggered != self._triggered:
            if self._sequence is None:
                raise RuntimeError('Sequence not set.')
            else:
                self.setSequence(self._sequence, loops, triggered)
        else:
            #self.proxy.stream(start='IMMEDIATE')
            self.startNow()
    ...
    def convert_sequence(self, sequence):
        ...
        return [(int(t), channels, 0, 0) for channels, t in sequence]

    def setContinuous(self, channels=[]):
        self.constant(pulse=(0, channels, 0, 0))
    ...
    def Light(self):
        self.constant(pulse=(0, ['repump'], 0, 0))
