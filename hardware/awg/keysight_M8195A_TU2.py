...
class AWGM8195A(Base, PulserInterface):
    ...
    pulsed_file_dir = ConfigOption(name='pulsed_file_dir', missing='warn')
    awg_mode = ConfigOption(name='awg_mode', default='FOUR', missing='warn')
    # debug = []

    _sample_rate_div = 4  # TODO grab from config
    ...
    def test(self):
        ...
        data = np.zeros(25600).astype('int8')

        bytes_written, enum_status_code = self.awg.write(':FUNC:MODE ARB')

        bytes_written, enum_status_code = self.awg.write(':INSTrument:DACMode FOUR')

        print('001', self.awg.query(':SYST:ERR?').strip().strip('"'))

        command = ':TRAC:DEL 1'
        bytes_written, enum_status_code = self.awg.write(command)
        print('002', self.awg.query(':SYST:ERR?').strip().strip('"'))

        command = ':TRAC:DEL 1'
        bytes_written, enum_status_code = self.awg.write(command)
        print('0021', self.awg.query(':SYST:ERR?').strip().strip('"'))

        command = ':TRAC{0}:DEF {1}, {2}, {3}'.format(1, 1, len(data), 0)
        bytes_written, enum_status_code = self.awg.write(command)
        print('003', self.awg.query(':SYST:ERR?').strip().strip('"'))

        command = ':TRAC{0}:DATA {1}, {2},'.format(1, 1, 0)
        bytes_written, enum_status_code = self.awg.write_binary_values(command, datatype='b', is_big_endian=False,
                                                                  values=data)
        print('004', self.awg.query(':SYST:ERR?').strip().strip('"'))

        command = ':TRAC{0}:DEF {1}, {2}, {3}'.format(2, 1, len(data), 0)
        bytes_written, enum_status_code = self.awg.write(command)
        print('0071', self.awg.query(':SYST:ERR?').strip().strip('"'))

        command = ':TRAC{0}:DATA {1}, {2},'.format(2, 1, 0)
        bytes_written, enum_status_code = self.awg.write_binary_values(command, datatype='b', is_big_endian=False,
                                                                  values=data)
        print('006', self.awg.query(':SYST:ERR?').strip().strip('"'))

        command = ':TRAC{0}:DEF {1}, {2}, {3}'.format(3, 1, len(data), 0)
        bytes_written, enum_status_code = self.awg.write(command)
        print('0061', self.awg.query(':SYST:ERR?').strip().strip('"'))

        command = ':TRAC{0}:DATA {1}, {2},'.format(3, 1, 0)
        bytes_written, enum_status_code = self.awg.write_binary_values(command, datatype='b', is_big_endian=False,
                                                                  values=data)
        print('007', self.awg.query(':SYST:ERR?').strip().strip('"'))

        command = ':TRAC{0}:DEF {1}, {2}, {3}'.format(4, 1, len(data), 0)
        bytes_written, enum_status_code = self.awg.write(command)
        print('0071', self.awg.query(':SYST:ERR?').strip().strip('"'))

        command = ':TRAC{0}:DATA {1}, {2},'.format(4, 1, 0)
        bytes_written, enum_status_code = self.awg.write_binary_values(command, datatype='b', is_big_endian=False,
                                                                  values=data)
        print('008', self.awg.query(':SYST:ERR?').strip().strip('"'))
    ...
    def _init_device(self):
        ...
        #TODO Probably add loading this options from the config file
        """
        ...
        self.write(':INSTrument:DACMode {0}'.format(self.awg_mode))   # TODO grab this setting from config
        ...
        # Set the directory:
        self.write(':MMEM:CDIR "C:\\Software\\pulsed_files"') # TODO from config

        self.sample_rate = self.get_sample_rate()

        ampl = {'a_ch1': 1.0, 'a_ch2': 1.0, 'a_ch3': 1.0, 'a_ch4': 1.0}
        #ampl = {'a_ch1': 1}
        d_ampl_low = {'d_ch1': 0, 'd_ch2': 0}           # changed by Philipp 06/07/2019 from 0.5 - 1.5
        d_ampl_high = {'d_ch1': 1, 'd_ch2': 1}          #TODO catch from config

        self.amplitude_list, self.offset_list = self.set_analog_level(amplitude=ampl)
        print(d_ampl_low, d_ampl_high)
        self.markers_low, self.markers_high = self.set_digital_level(low=d_ampl_low, high=d_ampl_high)
        print(self.markers_low, self.markers_high)
        ...
        constrains = self.get_constraints()
        self.set_sample_rate(constrains.sample_rate.default)
    ...
    def get_constraints(self):
        ...
        if self._MODEL == 'M8195A':
            awg_mode = self.query(':INST:DACM?') #TODO Could be replaced with self.awg_mode
            if awg_mode == 'MARK':
    ...
    def pulser_on(self):
        ...
        """
        self.write(':OUTP1 ON')
        self.write(':OUTP2 ON') #FIXME set dependend on awg mode
        self.write(':OUTP3 ON')
        self.write(':OUTP4 ON')
    ...
    def load_sequence(self, sequence_name):
        ...
        self.write(':STAB:SEQ:SEL {0}'.format(0))
        ...
        # path = self.pulsed_file_dir + self.sequence_dir
        #
    ...
    def write_waveform(self, name, analog_samples, digital_samples, is_first_chunk, is_last_chunk,
        ...
        for channel_index, channel_number in enumerate(active_analog):
            ...
            a_samples = (((analog_samples[channel_number] + max_ampl)/(2*max_ampl) * 255) - 128).astype('int8') #TODO insert Vpp here
            if marker and channel_number == 'a_ch1':
                d_samples = digital_samples['d_ch1'].astype('int8')+2*digital_samples['d_ch2'].astype('int8') #TODO channels are hardcoded
                ...
                interleaved_samples = a_samples   # TODO use interleaved_samples
            ...
            #self.write(':TRAC:DEL {0}'.format(segment_id)) der hier machts kaputt, ohne gehts
    ...
    def write_sequence(self, name, sequence_parameters_list):
        ...
        # TODO startpunkt festlegen (funktion), ueberpruefen ob min 257 samples da sind, endpunkt zum stoppen festlegen
        # TODO Wartezeit zwischen ssegmenten ueberpruefen
        """
        ...
        for step, (wfm_tuple, seq_step) in enumerate(sequence_parameters_list, 1):
            ...
            # print('control')
            # print(control)
            # print('seq_loop_count')
            # print(seq_loop_count)
            # print('seg_loop_count')
            # print(seg_loop_count)
            # print('segment_id')
            # print(segment_id)
            # print('seg_start_offset')
            # print(seg_start_offset)
            # print('seg_end_offset')
            # print(seg_end_offset)
            # print('Hier hoerts auf')
            ...
            # print('segment')
            # print(segment_id)
            # print(self.query(':STAB1:DATA? 1, 6'))
            ...
            print('999', self.query(':SYST:ERR?'))

        #print('hier', self.query(':TRAC:NAME? 1'))
        print('999', self.query(':SYST:ERR?'))
    ...
    def get_waveform_names(self):
        ...
        if awg_string != '0,0':
            ...
            print('length awg_string',len(awg_string))
            print('awg_string',awg_string)
            print('type',type(awg_string))
            print('zweiter eintrag',awg_string[2])

            awg_list = awg_string.split(",")
            print('laenge nach split',len(awg_list))

            for kk in range(int(len(awg_list)/2)):

                print('jeder zweite eintrag als integer',int(awg_list[2*kk]))
                print(':TRAC:NAME? {0}'.format(int(awg_list[2*kk])))
                waveform_list.append(self.query(':TRAC:NAME? {0}'.format(int(awg_list[2*kk]))))

            print('waveform liste',waveform_list)
        # try:
        ...
        #waveform_list.append(self.query(':TRAC:NAME? {0}'.format(self.query(':TRAC:SEL?'))))

        # # get only the files from the dir and skip possible directories
        # log = os.listdir(self.pulsed_file_dir)
        # file_list = list()
        #
        # for line in log:
    ...
    def delete_waveform(self, waveform_name):
        ...
        for name in waveform_name:
            for waveform in avail_waveforms:
                if fnmatch(waveform, name+'_ch?.bin8'):
    ...
    def delete_sequence(self, sequence_name):
        ...
        for sequence in sequence_name:
            for sequence in avail_sequences:
                if fnmatch(sequence, name+'_ch?.bin8'): # TODO hier passt was nicht
