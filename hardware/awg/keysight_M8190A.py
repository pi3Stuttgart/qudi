...
class AWGM8190A(Base, PulserInterface):
    ...
    def get_constraints(self):
        ...
        constraints.sequence_option = SequenceOption.OPTIONAL # not sure it is correct, but most probably it is
    ...
    def write_sequence(self, name, sequence_parameters):
        ...
        print('It is not implementer in QUDI')
        self.log.error('It is not implemented in QUDI, assemble your sequence somewhere else')
        # Generate the waveform segments using the Arbitrary Waveform Commands (TRACe Subsystem).
        # Define a sequence(: SEQ[1 | 2]:DEF: NEW).
        # self.define_new(num_steps)
...
# ===========================================================
    def define_new(self, length):
        ...
        :return:
        """
        # if length=:  # awg does not throw an error itself (which it does in define_segment())
        ...
        for ch in channel_numbers:
            sid.append(int(self.awg.query("SEQ{}:DEF:NEW? {}".format(ch, length))))
        return sid #Sequence(self.awg, self.channel_number, sid, sequences=self)
