...
class ODMR(DataGeneration):
    ...
    def run(self, abort, **kwargs):
        ...
        self.parameters['mw_mixing_freq'] = [pi3d.tt.mw_mixing_frequency]
        self.parameters['mw_mixing_freq_p1'] = [pi3d.tt.mw_mixing_frequency_p1]
        ...
        try:
            ...
            if transition == 'left':
                self.f_mhz = pi3d.tt.mw_mixing_frequency
            elif transition == 'right':
                self.f_mhz = pi3d.tt.mw_mixing_frequency_p1
            ...
            logging.getLogger().info(
                'ODMR duration {}s ({} runs)'.format((datetime.datetime.now() - pi3d.odmr.start_time).seconds,
                                                     getattr(getattr(pi3d.timetagger, __COUNTER_NAME__),
                                                             run_val_fun_name)()))
    ...
    def fit_and_update(self, abort):
        ...
        freq_list = np.array(pi3d.odmr.data.df.center_frequency.unique())
        ...
        if self.update_tt:
            if t == 'left':
                pi3d.tt.mw_mixing_frequency = self.f_mhz
                ...
                pi3d.tt.mw_mixing_frequency_p1 = self.f_mhz # TODO, we need to do a setter for the mw_mixing_frquency_p1
        ...
        if self.track_file[t] is not None:
            if t == 'left':
                pi3d.save_value_to_file(pi3d.tt.mw_mixing_frequency, self.track_file[t])
    ...
    def get_counts(self):
        ...
        getattr(pi3d.timetagger, __COUNTER_NAME__).sync()
        cts = np.array(getattr(pi3d.timetagger, __COUNTER_NAME__).getData()[0:,0], dtype=np.int32) if __COUNTER_NAME__ == 'odmr_timedifferences' \
            else np.array(getattr(pi3d.timetagger, __COUNTER_NAME__).getData()[0, :-1], dtype=np.int32) # last data value takes photons during possible RF pi pulse (13c90)
        cts2 = cts
        # time.sleep(self.readout_interval)
    ...
    def setup_counter(self):
        ...
        if __COUNTER_NAME__ == 'gated_counter_countbetweenmarkers':
            ...
            pi3d.timetagger.init_counter(__COUNTER_NAME__,
    ...
    def set_ret_mcas(self, val=None):
        def ret_mcas(current_iterator_df, sequence_name):
            ...
            for idx, _I_ in current_iterator_df.iterrows():
                frequencies = _I_['center_frequency'] + pi3d.tt.mfl(ast.literal_eval(_I_['td']), ms_trans=s)
                if idx%2 ==0:
                    factor = 0
                else:
                    factor = 5
