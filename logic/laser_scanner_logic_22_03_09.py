...
class LaserScannerLogic(GenericLogic):
    ...
    def on_activate(self):
        ...
        self._goto_speed = 10  # .01  # volt / second
        self.set_scan_speed(self._scan_speed)
        self._smoothing_steps = 1  # steps to accelerate between 0 and scan_speed
    ...
    def goto_voltage(self, volts=None):
        ...
        print(volts, 'volts in laserscannerlogic right before emitAAAAAAAAAAAAAAAAA')
    ...
    def _change_voltage(self, new_voltage):
        ...
        @return int: error code (0:OK, -1:error)
        """
        print("change voltageAAAAAAAAAAAAAAAAAAAA ", new_voltage)
    ...
    def set_scan_range(self, scan_range):
        """ Set the scan range """
        print(scan_range,'RAAAAAAAAAAANGE') #UNFUG
        r_max = np.clip(scan_range[1], -3, 3)
        r_min = np.clip(scan_range[0], -3, 3)
        ...
        print(self.scan_range,'SELF:RAAAAAAAAAAANGE') #UNFUG
    ...
    def start_scanning(self, v_min=None, v_max=None):
        ...
        @return int: error code (0:OK, -1:error)
        """
        print("(v_min, v_max, self._scan_speed)",(v_min, v_max))
        print(self.scan_range,'SELF_START:RAAAAAAAAAAANGE') #UNFUG
        ...
        self._upwards_ramp = self._generate_ramp(v_min, v_max, self._scan_speed) #all three values in [V].
        self._downwards_ramp = self._generate_ramp(v_max, v_min, self._scan_speed)

        print("######## upwards ramp not tampered=", self._upwards_ramp[3], self._upwards_ramp[3])
    ...
    def _do_next_line(self):
        ...
        """
        print("we are doing the next line")
        # stops scanning
    ...
    def _generate_ramp(self, voltage1, voltage2, speed):
        ...
        linear_v_step = speed / self._clock_frequency#QUICKFIX
        num_of_linear_steps = np.rint((v_max - v_min) / linear_v_step)#QUICKFIX
        ramp = np.linspace(v_min, v_max, num_of_linear_steps)#QUICKFIX
    ...
    def _scan_line(self, line_to_scan=None):
        ...
        try:
            ...
            #print("######## upwards ramp btampered=", self._upwards_ramp[3]) #UNFUG
            #print("###############line_to_scan before=",line_to_scan[3]) #UNFUG
