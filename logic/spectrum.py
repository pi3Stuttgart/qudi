...
class SpectrumLogic(GenericLogic):
    ...
    pulser = Connector(interface='PulserClient', optional=True)
    # cwavelaser = Connector(interface='CwaveLaser')

    # declare status variables
    _use_fine_spectrometer = False
    _automatic_flip = False
    _automatic_flip_mirror = True
    ...
    def on_activate(self):
        ...
        # self._spectrometer_fine = self.spectrometer_fine()
        #self._ello_flipper = self.ello_devices().ello_flip
        # self._cwave = self.cwavelaser()
        # self.sig_cwave_shutter.connect(self._cwave.set_shutters_states)
    ...
    def flip_mirror(self, mode = True):
        ...
        if mode:
            self._pulser.set_channel_on(channels=['ch6'])
        else: 
            self._pulser.set_channel_off(channels=['ch6'])
    @automatic_flip
