...
class CounterLogic(GenericLogic):
    ...
    def startCorr(self, binwidth, n_values):
        """ 
            @return error: 0 is OK, -1 is error
        """
        ...
        with self.threadlock:
            ...
            self._counting_device.set_up_correlation(int(binwidth/1e9*1e12), n_values)
            self.corr_x, self.corr_y = self._counting_device.get_correlation()
    ...
    def pause_resume_corr(self):
        with self.threadlock:
            ...
            if self.corr_stopRequested:
                ...
                self._counting_device.set_up_correlation(self.binwidth/1e9*1e12, self.n_values) #STOPCORRELATIOPN
                self.corr_stopRequested = False
                if self.module_state() == 'locked':
                    self.module_state.unlock()
                return

            # self._counting_device.corr.start() STPCORRELATIOPN
            self.module_state.lock()
        self.sigCountCorrNext.emit()
        return
