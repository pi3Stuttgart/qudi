...
class TimeTagger_simple():
    def __init__(self):
        self.t = TimeTagger.createTimeTagger()
    def init_counter(self, method, **kwargs):
        if method == 'counter':
            self.g_start_ch = TimeTagger.DelayedChannel(self.t, TimeTagger_channel_map['ssr_trigger'], int(0))
            self.g_stop_ch = TimeTagger.DelayedChannel(self.t, TimeTagger_channel_map['ssr_trigger'], int(1e3))
            self.gated_clicks = TimeTagger.GatedChannel(tagger=self.t,
                                                   input_channel=TimeTagger_channel_map['apd1'],
                                                   gate_start_channel=self.g_start_ch.getChannel(),
                                                   # TimeTagger_channel_map['ssr_trigger'],
                                                   gate_stop_channel=self.g_stop_ch.getChannel(),
                                                   # TimeTagger_channel_map['freq_swap_trigger'],
                                                   )

            self.counter =  TimeTagger.Counter(self.t,
                                      channels=[self.gated_clicks.getChannel()],
                                      binwidth=int(kwargs['interval_ps']),
                                      n_values=(kwargs['trace_length']))
...
def init_counter(kind, tagger, **kwargs):
    ...
    if kind == 'gated_counter_countbetweenmarkers':
        ...
        g_start_ch = TimeTagger.DelayedChannel(tagger, TimeTagger_channel_map['ssr_trigger'], int(0))
        g_stop_ch = TimeTagger.DelayedChannel(tagger, TimeTagger_channel_map['ssr_trigger'], int(1e5))
        gated_clicks = TimeTagger.GatedChannel(tagger = tagger,
                                               input_channel=TimeTagger_channel_map['apd1'],
                                               gate_start_channel=g_start_ch.getChannel(),#TimeTagger_channel_map['ssr_trigger'],
                                               gate_stop_channel=g_stop_ch.getChannel(),#TimeTagger_channel_map['freq_swap_trigger'],
                                               )
        trigge_levl = tagger.getTriggerLevel(3)
        print(trigge_levl, "trigger level")
        ...
        gated_clicks = TimeTagger.GatedChannel(tagger=tagger,
                                               input_channel=TimeTagger_channel_map['apd1'],
                                               gate_start_channel=g_start_ch.getChannel(),
                                               # TimeTagger_channel_map['ssr_trigger'],
                                               gate_stop_channel=g_stop_ch.getChannel(),
                                               # TimeTagger_channel_map['freq_swap_trigger'],
                                               )
