...
class NuclearOPs(DataGeneration):
    ...
    def run_measurement(self, abort, **kwargs):
        ...
        try:
            ...
            if pi3d.wavemeter.wm_id != 0 and self.wavemeter_lock:
            ...
            for idx, _ in enumerate(self.iterator()):
                ...
                while True:
                    ...
                    if 'aom_Ex_power_sweep' in self.current_iterator_df.keys():
                        print('I set voltage Ex: ', self.current_iterator_df['aom_Ex_power_sweep'].unique())
                        #do reach value here
                        pi3d.power_calibration.aom_list['aom_Ex_power'].set_voltage(self.current_iterator_df['aom_Ex_power_sweep'].unique())


                    if 'aom_A1_power_sweep' in self.current_iterator_df.keys():
                        print('I set voltage A1:  ', self.current_iterator_df['aom_A1_power_sweep'].unique())
                        time.sleep(0.1)
                        #do reach value here
                        pi3d.power_calibration.aom_list['aom_A1_power'].set_voltage(self.current_iterator_df['aom_A1_power_sweep'].unique())

                    if self.do_interferometerPhase_locking:
                        # print('Nuclear Ops, interferometer_locking1')
                        interferometer_phase = None
                        # print('Nuclear Ops, interferometer_locking2')

                        if 'interferometer_phase' in self.current_iterator_df.keys():
                            # print('self.current_iterator_df[interferometer_phase].unique()',self.current_iterator_df['interferometer_phase'].unique()[0])
                            interferometer_phase = self.current_iterator_df['interferometer_phase'].unique()[0]

                        # print('Nuclear Ops, interferometer_locking3')
                        self.do_interf_phase_lock(interferometer_phase)
                        # print('Nuclear Ops, interferometer_locking4')

                    if abort.is_set(): break
                    self.data.set_observations([OrderedDict(start_time=datetime.datetime.now())]*self.number_of_simultaneous_measurements)
                    self.data.set_observations([OrderedDict(mw_mixing_frequency=pi3d.tt.mw_mixing_frequency)]*self.number_of_simultaneous_measurements)
                    self.data.set_observations([OrderedDict(ple_Ex=pi3d.tt.ple_Ex)]*self.number_of_simultaneous_measurements)
                    self.data.set_observations([OrderedDict(ple_A1=pi3d.tt.ple_A1)]*self.number_of_simultaneous_measurements)
                    ...
                    pi3d.md['red_Ex'].run()
                    self.data.set_observations([OrderedDict(aom_Ex_power_measured=pi3d.power_calibration.pd_list['pd_Ex_power'].get_data())]*self.number_of_simultaneous_measurements)
                    time.sleep(0.1)
                    pi3d.md.stop_awgs()
                    ...
                    if pi3d.gated_counter.ZPL_counter:
                        for i, delay_ps in enumerate(self.delay_ps_list):
                            for j, window_ps in enumerate(self.window_ps_list):
                                name = 'zpl_counter_data_{i}_{j}'.format(i=i, j=j)
    ...
    def do_interf_phase_lock(self, interferometer_phase = None):
        ...
        if interferometer_phase is not None:
            # print('1 interferometer_phase: ',interferometer_phase)
            interferometer.desired_phase = interferometer_phase
            # print('do_interf_phase_lock 2')
        interferometer.do_calibration_scan()
        # print('do_interf_phase_lock 3')
        while interferometer.syncFlag == False:
            time.sleep(0.1)

        # print('do_interf_phase_lock 4')
    ...
    def get_trace(self, abort, delay_ps_list = None,window_ps_list = None):
        ...
        pi3d.gated_counter.count(abort,
    ...
    def save_sequence_file(self):
        seq_message = []
        for k in pi3d.md[self.mcas.name].sequences.keys():
            for ch in [1,2]:
                try:
                    seq_message.append(pi3d.md[self.mcas.name].sequences[k][ch].ret_info())
                except:
                    pass
