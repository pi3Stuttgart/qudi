...
def ret_ret_mcas(pdc):
    def ret_mcas(self, current_iterator_df, sequence_name=None):
        ...
        for idx, _I_ in current_iterator_df.iterrows():
            mcas.asc(A1=(_I_['readout'] == 'A1'), A2=(_I_['readout'] == 'A2'), length_mus=10., name='init')
            ...
            if _I_['readout'] == 'A2':
                sna.ssr(mcas=mcas, queue=self.queue, frequencies=freq, wait_dur=0.0, robust=False,
                        nuc='ple_A2', mixer_deg=-90, eom_ampl=0.0, step_idx=0, laser_dur=2.5)
            elif _I_['readout'] == 'A1':
                sna.ssr(mcas=mcas, queue=self.queue, frequencies=freq, wait_dur=0.0, robust=False,
                        nuc='ple_A1', mixer_deg=-90, eom_ampl=0.0, step_idx=0, laser_dur=2.5)
...
def settings(pdc={}):
    ...
    nuclear.parameters = OrderedDict(
        (
            ...
            ('sweeps', range(10)),
            ('readout', ['A2']),
            ('mw_duration', E.round_length_mus_full_sample(np.linspace(0.0, 0.2, 61))),
        )
    )
