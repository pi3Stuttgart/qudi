...
def ret_ret_mcas(pdc):
    def ret_mcas(current_iterator_df):
        ...
        for idx, _I_ in current_iterator_df.iterrows():
            mcas.start_new_segment(name='Electron Rabi')
            mcas.asc(green=True, length_mus = 3.0)
            mcas.asc(green=False, length_mus=0.5)
            # sna.electron_rabi(
            #     mcas,
            #     new_segment=True,
            ...
            # sna.ssr(mcas, frequencies=freq, wait_dur=0.0, robust=False, nuc='ple_Ex', mixer_deg=-90,
            #         step_idx=1, laser_dur=_I_['laser_dur_ssr'])
            sna.ssr(mcas, frequencies=freq, wait_dur=0.0, robust=False, nuc='charge_state', mixer_deg=-90,
                    step_idx=0, laser_dur=_I_['laser_dur_ssr'])
...
def settings(pdc):
    ...
    nuclear.parameters = OrderedDict(
        (
            ('sweeps', range(100)),
            ('transition', [0]),
            ('laser_dur_ssr', [50.0,100.0,150.0,200.0,600.0]),
            ('mixer_deg', [-90]),
            ('amp0', [0.1]),
            ('factor', [1]),

            # ('amp0', ["{:.6f}".format(i) for i in E.round_to_amplitude_granularity([i for i in np.linspace(0.0, 0.3, 20) if i != 0.0 and i > 0.15])]),
            # ('factor', np.linspace(0.0, 2, 19))
        )
    )
    ...
    nuclear.number_of_simultaneous_measurements = 3#len(nuclear.parameters['factor'])
    nuclear.ple_refocus_range = 4.0
    nuclear.scan_amounts = 5
    nuclear.refocus_step = 0.05
    nuclear.fit = False
    nuclear.do_ple_refocus = True
    nuclear.maximum_odmr_drift = 0.5 # MHz
    nuclear.odmr_interval = 600 # s
...
def run_fun(abort, **kwargs):
    ...
    if len(nuclear.parameters['amp0']) <= 2:
        print('WARNING: Not enough amplitudes. Will not be saved for TransitionTracker. {}'.format(nuclear.parameters['amp0']))
    nuclear.run(abort)
